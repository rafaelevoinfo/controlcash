...
public class Node {
	...
	private Node(String s, Expression exp) {
		s = s.replaceAll("[" + String.valueOf(' ') + "]", "");// alterado by Rafael, apenas para não ter que
																// usar a classe StringUtils
		s = removeBrackets(s);
		s = addZero(s);
		if (!checkBrackets(s))
			throw new IllegalArgumentException("Wrong number of brackets in '" + s + "'");
		...
		if (operator != null) {
			...
			if (startOperator == 0 && operator.getType() == Operands.SINGLE) {
				if (checkBrackets(s.substring(operator.getOperator().length()))) {
					leftNode = new Node(s.substring(operator.getOperator().length()), exp);
					return;
				}
				throw new IllegalArgumentException("Error parsing. Missing brackets in '" + s + "'");
			} else if (startOperator > 0 && operator.getType() == Operands.DOUBLE) {
				leftNode = new Node(s.substring(0, startOperator), exp);
				rightNode = new Node(s.substring(startOperator + operator.getOperator().length()), exp);
			}
		}
	}
